// Batch F (fast policies) applier with verification.  node scripts/batch_F_apply.mjs dryrun|apply
...
const sql=fs.readFileSync('sql/batch_F_fast_policies.sql','utf8').replace(/^BEGIN;/m,'').replace(/COMMIT;\s*$/m,'');
await c.query('BEGIN');
try{
  ...
  const cat=(await c.query(`SELECT
    ...
    (SELECT count(*) FROM pg_policy p JOIN pg_class c ON c.oid=p.polrelid WHERE c.relnamespace='public'::regnamespace AND (COALESCE(pg_get_expr(p.polqual,p.polrelid),'')||COALESCE(pg_get_expr(p.polwithcheck,p.polrelid),'')) ~ 'is_school_(member|owner)\\(school_id\\)|school_is_active\\(school_id\\)') per_row_policies_left,
    (SELECT count(*) FROM pg_proc WHERE proname IN ('member_school_ids','owner_school_ids','active_school_ids') AND prosecdef) helpers,
    (SELECT count(*) FROM schools s WHERE NOT EXISTS (SELECT 1 FROM school_members m WHERE m.school_id=s.id AND m.user_id=s.user_id AND m.status='active')) schools_without_owner_row`)).rows[0];
  ok('RLS enabled on every public table', +cat.rls_off===0);
  ok('no per-row function policies remain', +cat.per_row_policies_left===0, `left=${cat.per_row_policies_left}`);
  ok('three set helpers exist (SECURITY DEFINER)', +cat.helpers===3);
  ok('every school owner has a membership row (boot = 1 query)', +cat.schools_without_owner_row===0, `missing=${cat.schools_without_owner_row}`);
  ok('policy count', +cat.pols<=+pre.pols, `${pre.pols} -> ${cat.pols}`);
  // timing: the same seven screen queries as the owner, EXPLAIN ANALYZE
  const S=fx.sid; const Q={students:`SELECT id, first_name, last_name, current_class_id, current_monthly_fee, active FROM students WHERE school_id='${S}' ORDER BY first_name`, parents_balances:`SELECT p.id, p.first_name, p.last_name, p.contact, b.balance FROM parents p LEFT JOIN parent_balances b ON b.parent_id=p.id WHERE p.school_id='${S}' ORDER BY p.first_name`, fee_rows:`SELECT student_id, month, net_amount FROM student_monthly_fees WHERE school_id='${S}'`, allocations:`SELECT a.* FROM payment_allocations a JOIN payments p ON p.id=a.payment_id WHERE p.school_id='${S}'`, ledger_all:`SELECT id, entry_type, amount, month FROM ledger WHERE school_id='${S}'`};
  await as(fx.owner); const times={}; for (const [k,q] of Object.entries(Q)) { await c.query(q); const r=await c.query('EXPLAIN (ANALYZE, FORMAT JSON) '+q); times[k]=+r.rows[0]['QUERY PLAN'][0]['Execution Time'].toFixed(2); } await reset();
  console.log('owner query times after rewrite (ms):', JSON.stringify(times));
  ok('largest screen query under 3 ms in the database', Math.max(...Object.values(times))<3, `max=${Math.max(...Object.values(times))}`);
  ...
  await as(mgr); let r;
  r=await tryq(`SELECT m.role, s.school_name FROM school_members m JOIN schools s ON s.id=m.school_id WHERE m.user_id=$1 AND m.status='active'`,[mgr]); ok('mgr resolves membership + school in one query (boot)', r.ok && r.r.rowCount===1 && !!r.r.rows[0].school_name, r.ok?`rows=${r.r.rowCount}`:r.err);
  ...
  await as(fx.other_owner); r=await tryq(`SELECT count(*) n FROM parents WHERE school_id=$1`,[fx.sid]); ok('other school sees nothing', r.ok && +r.r.rows[0].n===0); r=await tryq(`SELECT count(*) n FROM schools`); ok('other owner sees only their own school row', r.ok && +r.r.rows[0].n===1, r.ok?`rows=${r.r.rows[0].n}`:r.err); await reset();
  ...
  const same=['st','pa','py','le','u'].every(k=>fin[k]===pre[k]) && +fin.sm===+pre.sm+3;
