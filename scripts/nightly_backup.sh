...
# Also uploads the zip to Baserow (second copy). GitHub Actions runs the same two steps nightly regardless of this Mac.
...
"$NODE" scripts/backup_to_baserow.mjs || echo "$(date '+%F %T') Baserow upload failed (local copy kept)"
